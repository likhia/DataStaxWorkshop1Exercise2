...
(B) Set the database information
(C) Walkthrough the codes 
(D) Test the REST services
...
## Section B: Set the database information 

1. Create a new file named as **.env** under the root folder.   Copy and paste the properties from .env.example and set its value from the Console tab and also the token from the generated token. 

2. These properties will be used by this application to connect to Astra DB.  


## Section C: Walkthrough the codes 

1. app.js contains the source codes for this application. 

2. The code below is to create the base url of the REST API to connect to Astra DB.   It returns the base URL for particular table. 
```
